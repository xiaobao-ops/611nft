...
import { generateNonce, sleep } from "@/utils";
import { readFile } from "fs/promises";
import { GAS_PREMIUM, ZERO_ADDRESS } from "@/utils/const";
...
let unique = 0;
export const runMine = async (accountInfo: IAccountInfo, options: IMineOptions) => {
  sayMinerLog();
  // const str = await readFile("./tokens.json", "utf-8");
  const { tick, amt, workc, rpc } = options;
  const {account, privateKey} = accountInfo

  // printer.trace(`Start mining with ${account}`);
  const provider = new ethers.providers.JsonRpcProvider(rpc);
  const miner = new ethers.Wallet(privateKey, provider);

  // printer.trace(`Start mining with ${await miner.getAddress()}`);
